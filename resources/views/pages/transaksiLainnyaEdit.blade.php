...
@section("dashboard-content")
    ...
    <div class="flex mb-2">
        <a href="/transaksi/lain-lain" class="text-sm hover:text-blue-500">Lain-lain</a>
        <div class="mx-1">/</div>
    ...
    >
        <div class="bg-white shadow-md rounded-lg w-9/12 sm:w-7/12 md:w-5/12 lg:w-4/12 scale-95 p-5 transition-transform duration-300 ease-out">
            ...
            <div class="flex justify-between">
                <span>Edit Penerimaan Lain-lain</span>
            </div>
            <hr class="my-4">
            <form action="/transaksi/lain-lain/edit/{{$lains->id}}" method="POST">
                ...
                <input id="nominal" type="number" name="nominal" class="border rounded-lg px-3 py-2 mt-1 mb-5 text-sm w-full" value="{{$lains->nominal}}" @required(true)/>
                <label for="tgl" class="font-semibold text-sm text-gray-600 pb-1 block">Tanggal</label>
                <input type="date" name="tgl" id="tgl" class="border rounded-lg px-3 py-2 mt-1 mb-5 text-sm w-full" value="{{$lains->tanggal}}">
