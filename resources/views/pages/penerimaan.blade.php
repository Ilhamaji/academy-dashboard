...
@section('dashboard-content')
    ...
    <div class="flex">
        ...
        <span class="inline my-auto ml-2 font-bold">Tambah Data Penerimaan</span>
    </div>

    <div class="flex">
        @include('components.modalTambahJenisPenerimaan')
    </div>
          ...
          <div class="p-1.5 min-w-full inline-block align-middle">
            <div class="overflow-hidden">
              <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody>
                @foreach ($jenis_penerimaan as $jenis)
                    <tr class="odd:bg-white even:bg-gray-100">
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $loop->iteration }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $jenis->kode }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-500">{{ $jenis->kategori }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-500">{{ $jenis->nama }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-end text-sm font-medium flex justify-self-end">
                            <a href="/penerimaan/edit/{{$jenis->kode}}" class="cursor-pointer bg-blue-500 hover:bg-blue-700 p-3 rounded-lg">
